...
/* T in Array<T> is used for allowing typesafety. so basically we don't know what kind of array we 
   put in here.but whatever kind it will typesafe */
...
function ReviewColumn({
    ...
    /* for get the column height */
    const [columnHeight, setColumnHeight] = useState(0)
    const duration=`${columnHeight * msPerPixel}ms`
    /* for listen to resize event */
    useEffect(() => {
...
function ReviewGrid(){
    ...
    /* once:true for animate only once */
    /* amount:0.4 when user see the content */
